...
class OutletTypeAdapter(
    private var mActivity: Activity,
    private var clickListener: OnItemClickListener
) :
    RecyclerView.Adapter<OutletTypeAdapter.ViewHolder>() {

    private var arrayList = listOf<OutletTypeMasterDTO>()

    fun submitList(updatedList: List<OutletTypeMasterDTO>) {
    ...
    class ItemDiffCallback(
        private var oldList: List<OutletTypeMasterDTO>,
        private var newList: List<OutletTypeMasterDTO>
    ) : DiffUtil.Callback() {
        override fun areItemsTheSame(oldItemPosition: Int, newItemPosition: Int): Boolean {
            return (oldList[oldItemPosition].outletTypeId == newList[oldItemPosition].outletTypeId)
        }
    ...
    class ViewHolder(private val binding: BasicInfoMastersItemCellLayoutBinding) :
        ...
        fun bind(
            activity: Activity,
            position: Int,
            dto: OutletTypeMasterDTO,
            itemClick: OnItemClickListener
        ) {

            binding.text = dto.outletTypeName ?: ""
        ...
        companion object {
            fun from(parent: ViewGroup): ViewHolder {
                ...
                val binding =
                    BasicInfoMastersItemCellLayoutBinding.inflate(
                        layoutInflater,
                        parent,
                        false
                    )
    ...
    interface OnItemClickListener {
        fun onItemClick(
            position: Int,
            outletTypeMasterDTO: OutletTypeMasterDTO
        )
    }
